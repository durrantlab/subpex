...
class NewWeightEntry:
    ...
    def __init__(
        ...
        self.prev_init_pcoord = (
            numpy.asarray(prev_init_pcoord) if prev_init_pcoord is not None else None
        )
        self.prev_final_pcoord = (
            numpy.asarray(prev_final_pcoord) if prev_final_pcoord is not None else None
        )
        self.new_init_pcoord = (
            numpy.asarray(new_init_pcoord) if new_init_pcoord is not None else None
        )
...
class WEDriver:
    ...
    def process_config(self):
        ...
        log.info(
            "Adjust counts to exactly match target_counts: {}".format(
                self.do_adjust_counts
            )
        )

        self.weight_split_threshold = config.get(
            ["west", "we", "weight_split_threshold"], self.weight_split_threshold
        )
        log.info("Split threshold: {}".format(self.weight_split_threshold))

        self.weight_merge_cutoff = config.get(
            ["west", "we", "weight_merge_cutoff"], self.weight_merge_cutoff
        )
    ...
    def next_iter_segments(self):
        ...
        if self.next_iter_binning is None:
            raise RuntimeError(
                "cannot access next iteration segments before running WE"
            )
    ...
    def new_iteration(
        ...
        flux_matrix = self.flux_matrix = numpy.zeros(
            (nbins, nbins), dtype=numpy.float64
        )
        transition_matrix = self.transition_matrix = numpy.zeros(
            (nbins, nbins), numpy.uint
        )
        ...
        for tstate in target_states:
            ...
            log.debug(
                "target state {!r} mapped to bin {}".format(tstate, tstate_assignment)
            )
            self.bin_target_counts[tstate_assignment] = 0

        # loop over recycled segments, adding entries to the flux matrix appropriately
        if new_weights:
            init_pcoords = numpy.empty(
                (len(new_weights), self.system.pcoord_ndim),
                dtype=self.system.pcoord_dtype,
            )
            prev_init_pcoords = numpy.empty(
                (len(new_weights), self.system.pcoord_ndim),
                dtype=self.system.pcoord_dtype,
            )
            ...
            for entry, i, j in zip(
                new_weights, prev_init_assignments, init_assignments
            ):
    ...
    def assign(self, segments, initializing=False, endprop=False, recycling=False):
        ...
        if recycling:
            ...
            for iseg, segment in enumerate(segments):
                all_pcoords[0, iseg] = numpy.append(
                    segment.pcoord[0, :], segment.weight
                )
                all_pcoords[1, iseg] = numpy.append(
                    segment.pcoord[-1, :], segment.weight
                )
        if endprop:
        ...
        for segment, iidx, fidx in zip(
            segments, initial_assignments, final_assignments
        ):
        ...
        for ibin, target_state in self.target_states.items():
            ...
            for segment in set(target_bin):
                ...
                if log.isEnabledFor(logging.DEBUG):
                    log.debug(
                        "recycling {!r} from target state {!r} to initial state {!r}".format(
                            segment, target_state, initial_state
                        )
                    )
        ...
        for segment in segments:
            if segment is gparent_seg:
                ...
                if segment.parent_id >= 0:
                    self._parent_map[segment.parent_id].endpoint_type = (
                        Segment.SEG_ENDPOINT_CONTINUES
                    )
            else:
                ...
                if segment.parent_id >= 0:
                    self._parent_map[segment.parent_id].endpoint_type = (
                        Segment.SEG_ENDPOINT_MERGED
                    )
                else:
                    if segment.initial_state_id in {
                        segment.initial_state_id for segment in bin
                    }:
                        ...
                        log.debug(
                            "freeing initial state {!r} for future use (merged)".format(
                                initial_state
                            )
                        )
                        self.avail_initial_states[initial_state.state_id] = (
                            initial_state
                        )
    ...
    def _split_by_weight(self, ibin):
        ...
        segments = numpy.array(
            sorted(bin, key=operator.attrgetter("weight")), dtype=numpy.object_
        )
    ...
    def _check_pre(self):
        for ibin, _bin in enumerate(self.next_iter_binning):
            if self.bin_target_counts[ibin] == 0 and len(_bin) > 0:
                raise ConsistencyError(
                    "bin {:d} has target count of 0 but contains {:d} walkers".format(
                        ibin, len(_bin)
                    )
                )
    ...
    def populate_initial(self, initial_states, weights, system=None):
        ...
        if abs(1.0 - tprob) > len(weights) * EPS:
            pscale = 1.0 / tprob
            log.warning(
                "Weights of initial segments do not sum to unity; scaling by {:g}".format(
                    pscale
                )
            )
            for segment in segments:
        ...
        for segment in self.next_iter_segments:
            ...
            try:
                self.used_initial_states[istate.state_id] = (
                    self.avail_initial_states.pop(istate.state_id)
                )
            except KeyError:
    ...
    def _log_bin_stats(self, bin, heading=None, level=logging.DEBUG):
        if log.isEnabledFor(level):
            ...
            log_msg = log_fmt.format(
                ...
                median_weight=numpy.median(weights),
