...
import west
import westpa
from west import Segment, wm_ops
...
class WESimManager:
    def process_config(self):
        ...
        self.propagator_block_size = config.get(
            ["west", "propagation", "block_size"], 1
        )
        self.save_transition_matrices = config.get(
            ["west", "propagation", "save_transition_matrices"], False
        )
        self.max_run_walltime = config.get(
            ["west", "propagation", "max_run_wallclock"], default=None
        )
        self.max_total_iterations = config.get(
    ...
    def __init__(self, rc=None):
        ...
        self.next_iter_bstate_cprobs = (
            None  # Cumulative probabilities for basis states, used for selection
        )
        ...
        self.segments = (
            None  # Mapping of seg_id to segment for all segments in this iteration
        )
    ...
    def report_bin_statistics(self, bins, save_summary=False):
        ...
        seg_probs = numpy.fromiter(
            map(operator.attrgetter("weight"), segments),
            dtype=weight_dtype,
            count=len(segments),
        )
        bin_probs = numpy.fromiter(
            map(operator.attrgetter("weight"), bins),
            dtype=weight_dtype,
            count=len(bins),
        )
        norm = seg_probs.sum()
        ...
        self.rc.pstatus(
            "{:d} of {:d} ({:%}) active bins are populated".format(
                n_pop, n_active_bins, n_pop / n_active_bins
            )
        )
        self.rc.pstatus(
            "per-bin minimum non-zero probability:       {:g}".format(min_bin_prob)
        )
        self.rc.pstatus(
            "per-bin maximum probability:                {:g}".format(max_bin_prob)
        )
        self.rc.pstatus(
            "per-bin probability dynamic range (kT):     {:g}".format(bin_drange)
        )
        self.rc.pstatus(
            "per-segment minimum non-zero probability:   {:g}".format(min_seg_prob)
        )
        self.rc.pstatus(
            "per-segment maximum non-zero probability:   {:g}".format(max_seg_prob)
        )
        self.rc.pstatus(
            "per-segment probability dynamic range (kT): {:g}".format(seg_drange)
        )
        self.rc.pstatus(
            "norm = {:g}, error in norm = {:g} ({:.2g}*epsilon)".format(
                norm, (norm - 1), (norm - 1) / EPS
            )
        )
        self.rc.pflush()
        ...
        if save_summary:
            ...
            if numpy.isnan(iter_summary["cputime"]):
                iter_summary["cputime"] = 0.0
            if numpy.isnan(iter_summary["walltime"]):
    ...
    def get_bstate_pcoords(self, basis_states):
        ...
        futures = [
            self.work_manager.submit(wm_ops.get_pcoord, args=(basis_state,))
    ...
    def report_target_states(self, target_states):
        ...
        if self.rc.verbose_mode and target_states:
            pstatus(":")
            pstatus(
                "{:6s}    {:12s}    {}".format("ID", "Label", "Progress Coordinate")
            )
            for target_state in target_states:
                pstatus(
                    "{:<6d}    {:12s}    {}".format(
                        ...
                        target_state.label,
                        ",".join(map(str, target_state.pcoord)),
                    )
                )
        pstatus()
        self.rc.pflush()
    ...
    def initialize_simulation(
        self, basis_states, target_states, segs_per_state=1, suppress_we=False
    ):
        ...
        if self.do_gen_istates:
            futures = [
                work_manager.submit(
                    wm_ops.gen_istate, args=(initial_state.basis_state, initial_state)
                )
        ...
        bin_occupancies = numpy.fromiter(
            map(len, binning), dtype=numpy.uint, count=self.we_driver.bin_mapper.nbins
        )
        target_occupancies = numpy.require(
            self.we_driver.bin_target_counts, dtype=numpy.uint
        )
    ...
    def prepare_iteration(self):
        ...
        self.we_driver.new_iteration(
            target_states=target_states, new_weights=new_weights
        )
        ...
        if self.segments is None:
            segments = self.segments = {
                segment.seg_id: segment for segment in self.data_manager.get_segments()
            }
        ...
        log.debug(
            "{:d} segments are complete; {:d} are incomplete".format(
                len(completed_segments), len(incomplete_segments)
            )
        )
        ...
        self.rc.pstatus(
            "{:d} segments remain in iteration {:d} ({:d} total)".format(
                len(incomplete_segments), self.n_iter, len(segments)
            )
        )
        ...
        self.current_iter_istates = {
            ...
            for state in self.data_manager.get_segment_initial_states(
                list(segments.values())
            )
        }
        log.debug(
            "This iteration uses {:d} initial states".format(
                len(self.current_iter_istates)
            )
        )
        ...
        self.next_iter_bstate_cprobs = numpy.add.accumulate(
            [bstate.probability for bstate in self.next_iter_bstates]
        )

        self.we_driver.avail_initial_states = {
            istate.state_id: istate
            for istate in self.data_manager.get_unused_initial_states(
                n_iter=self.n_iter + 1
            )
        }
        log.debug(
            "{:d} unused initial states found".format(
                len(self.we_driver.avail_initial_states)
            )
        )
        ...
        self.work_manager.submit(
            wm_ops.prep_iter, args=(self.n_iter, segments)
        ).get_result()
    ...
    def finalize_iteration(self):
        ...
        self.work_manager.submit(
            wm_ops.post_iter, args=(self.n_iter, list(self.segments.values()))
        ).get_result()

        # Move existing segments into place as new segments
        del self.segments
        self.segments = {
            segment.seg_id: segment for segment in self.we_driver.next_iter_segments
        }
    ...
    def get_istate_futures(self):
        ...
        log.debug(
            "{:d} unused initial states available".format(
                len(self.we_driver.avail_initial_states)
            )
        )
        log.debug(
            "{:d} new initial states required for recycling {:d} walkers".format(
                n_istates_needed, n_recycled
            )
        )
        ...
        for _i in range(n_istates_needed):
            ...
            initial_state = self.data_manager.create_initial_states(
                1, n_iter=self.n_iter + 1
            )[0]
            ...
            if self.do_gen_istates:
                log.debug(
                    "generating new initial state from basis state {!r}".format(
                        basis_state
                    )
                )
                ...
                futures.add(
                    self.work_manager.submit(
                        wm_ops.gen_istate, args=(basis_state, initial_state)
                    )
                )
            else:
                ...
                initial_state.istate_status = InitialState.ISTATE_STATUS_PREPARED
                self.we_driver.avail_initial_states[initial_state.state_id] = (
                    initial_state
                )
    ...
    def propagate(self):
        ...
        log.debug(
            "iteration {:d}: propagating {:d} segments".format(
                self.n_iter, len(segments)
            )
        )
        ...
        for segment_block in grouper(self.propagator_block_size, segments):
            ...
            pbstates, pistates = west.states.pare_basis_initial_states(
                self.current_iter_bstates,
                list(self.current_iter_istates.values()),
                segment_block,
            )
            future = self.work_manager.submit(
                wm_ops.propagate, args=(pbstates, pistates, segment_block)
            )
            futures.add(future)
        ...
        while futures:
            ...
            if future in segment_futures:
                ...
                self.completed_segments.update(
                    {segment.seg_id: segment for segment in incoming}
                )
                ...
                log.debug(
                    "received newly-prepared initial state {!r}".format(initial_state)
                )
                initial_state.istate_status = InitialState.ISTATE_STATUS_PREPARED
                with self.data_manager.expiring_flushing_lock():
                    self.data_manager.update_initial_states(
                        [initial_state], n_iter=self.n_iter + 1
                    )
                self.we_driver.avail_initial_states[initial_state.state_id] = (
                    initial_state
                )
                ...
                log.error(
                    "unknown future {!r} received from work manager".format(future)
                )
    ...
    def check_propagation(self):
        ...
        if failed_segments:
            ...
            log.error(
                "propagation failed for {:d} segment(s):\n{}".format(
                    len(failed_segments), failed_ids
                )
            )
            raise PropagationError(
                "propagation failed for {:d} segments".format(len(failed_segments))
            )
        else:
        ...
        if failed_istates:
            ...
            log.error(
                "initial state generation failed for {:d} states:\n{}".format(
                    len(failed_istates), failed_ids
                )
            )
            raise PropagationError(
                "initial state generation failed for {:d} states".format(
                    len(failed_istates)
                )
            )
        else:
            log.debug(
                "initial state generation complete for iteration {:d}".format(
                    self.n_iter
                )
            )
    ...
    def run_we(self):
        ...
        if self.we_driver.used_initial_states:
            ...
            self.data_manager.update_initial_states(
                list(self.we_driver.used_initial_states.values())
            )
        ...
        self.data_manager.save_iter_binning(
            self.n_iter + 1, hashed, pickled, self.we_driver.bin_target_counts
        )
        ...
        tstates_by_id = {
            state.state_id: state for state in self.we_driver.target_states.values()
        }
        for tstate_id, weights in recycling_events.items():
            tstate = tstates_by_id[tstate_id]
            self.rc.pstatus(
                "Recycled {:g} probability ({:d} walkers) from target state {!r}".format(
                    sum(weights), len(weights), tstate.label
                )
            )
    ...
    def prepare_new_iteration(self):
        ...
        if self.rc.debug_mode:
            ...
            for segment in self.we_driver.next_iter_segments:
                self.rc.pstatus(
                    "{!r} pcoord[0]={!r}".format(segment, segment.pcoord[0])
                )

        self.data_manager.prepare_iteration(
            self.n_iter + 1, list(self.we_driver.next_iter_segments)
        )
        self.data_manager.save_new_weight_data(
            self.n_iter + 1, self.we_driver.new_weights
        )
    ...
    def run(self):
        ...
        if max_walltime:
            ...
            self.rc.pstatus(
                "Maximum wallclock time: %s" % timedelta(seconds=max_walltime or 0)
            )
        else:
            run_killtime = None
        ...
        while self.n_iter <= max_iter:
            if max_walltime and time.time() + 1.1 * iter_elapsed >= run_killtime:
                self.rc.pstatus(
                    "Iteration {:d} would require more than the allotted time. Ending run.".format(
                        self.n_iter
                    )
                )
                return
            ...
            try:
                ...
                self.rc.pstatus(
                    "Iteration wallclock: {0!s}, cputime: {1!s}\n".format(
                        walltime, cputime
                    )
                )
                self.rc.pflush()
